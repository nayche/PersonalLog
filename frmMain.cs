    ...
    public partial class frmMain : Form
    {
        ...
        private void bntSave_Click_1(object sender, EventArgs e)
        {
            try
            {
                try
                {
                    ...
                    cntPLog.Open();
                    daLog.Update(dstPLog1, "PersonalLog");
                    cntPLog.Close();
        ...
        private void btnModify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataRow dRowTemp = dstPLog1.PersonalLog.NewRow();    // In case the new modified row matches an existing row, this way it would not update the row in the dataset
                dRowTemp[0] = txtName.Text;
                dRowTemp[1] = txtDestination.Text;
                dRowTemp[2] = txtTimeOut.Text;
                dRowTemp[3] = txtReturnTime.Text;
                dRowTemp[4] = dateTimePLog.Value.ToShortDateString();


                // check against multiple row
                if (MultipleRecords(dRowTemp, dstPLog1.PersonalLog))
                {
                    Utilities.WarningMsgBox("This record already exists in the database.\n" +
                            "Data not saved");
                    dgvCellClickEventHandler = new DgvCellClickDelegate(dgvPLog_CellClick);
                    dgvCellClickEventHandler.Invoke(dgvPLog,new DataGridViewCellEventArgs(1,1));
       ...
       private void mnuClose_Click(object sender, EventArgs e)
        ...
        private void dgvPLog_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                //this.dateTimePLog.Value = (DateTime)this.dgvPLog.CurrentRow.Cells[4].Value;
            }
            catch (Exception ex)
            {
                Utilities.ErrorMsgBox(ex.Message);
        ...
        private void dgvPLog_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
